...
#def cross_validation(D, L, classifier_class, n_folds= 10, prep= False, dim_red= False, m_dim_red= False, seed= 0, args_in= {}):
#    N = D.shape[1]
#    np.random.seed(seed)
...
#    D_folds = []
#    L_folds = []
#    npf = ceil(N / n_folds)  ## cambiare a floor() se si preferisce l'ultima fold più grande delle altre piuttosto che più piccola
...
#        predictions.append(classifier.getPredictions(DTE))
#
#    return np.vstack(LTE), np.vstack(predictions)
...
def getMinNormDCF(scores_in, labels_in, p1, Cfp, Cfn, retPR= False):
    N = scores_in.shape[0]
    ...
    for i in range(scores.shape[0]):
        ...
        conf = getConfusionMatrix(predictions, labels)
        FPR, FNR, TPR, _ = getFR(conf)
        ...
        if nDCF < minDCF:
            minDCF = nDCF
    if retPR: return minDCF, (FPRs, TPRs)
    else: return minDCF

def inter_BayesErrorPlot(llr, labels, eff):
    FPR, FNR, _, _ = getFR(getConfusionMatrix((llr > getBayesThreshold(eff, 1, 1)).astype(int), labels))
    return normalizedDCF(eff, 1, 1, FPR, FNR), getMinNormDCF(llr, labels, eff, 1, 1)

def cross_validation(DTR_in, LTR_in, n_split, model, model_params, prepro= [[]], progress= False, save= False, filename= 'results\\cross_vall_no_name.txt', model_params_print= None, print_act= False):
