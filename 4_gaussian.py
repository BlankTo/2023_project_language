...
variant_range = [False, 'DIAG', 'TIED', 'DIAG-TIED']
            ...
            for mPCA in mPCA_range:
                ...
                for mLDA in mLDA_range:
                    ...
                    for variant in variant_range:
                        ...
                        scores_i.append(gau.Gaussian_Classifier(Dtr_l, Ltr, Dte_l, 0, 2, prior, variant, retScores= True))
                        place += 1
        ...
        for mPCA in mPCA_range:
            for mLDA in mLDA_range:
                ...
                for variant in variant_range:
                    ...
                    #np.save('scores_gaussian', scores)
                    #np.save('cons_gaussian', considered)
                    place += 1
                    ...
                    spec = str('(' + str('%.2f' % prior) + ', ' + str(prepre if prepre else 'raw  ') + ', ' + str(mPCA if mPCA==10 else (str(mPCA) + ' ' if mPCA else 'no')) + ', ' + str(str(mLDA) + ' ' if mLDA else 'no') + ', ' + str(variant if variant=='DIAG-TIED' else (str(variant) + '     ' if variant else 'FULL COV ')) + ')')
                    print_string = str(spec + '->   err 1/2: ' + str('%.2f' % correct02)
                                            + '  err eff: ' + str('%.2f' % correct)
                                            + '  |  minDCF 1/2: ' + str('%.3f' % mini05)
                                            + '  minDCF wff: ' + str('%.3f' % mini)
