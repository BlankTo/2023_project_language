...
save = True
newfile = False
filename = 'results/kernel_svm_rbf.txt'
progress = True
n_split = 100
            ...
            for mPCA in mPCA_range:
                ...
                for mLDA in mLDA_range:
                    ...
                    for lam in lambda_range:
                        kernel_f = svm.get_kernel_RBF(lam)
                        for C in C_range:
                            for K in K_range:
                ...
                for lam in lambda_range:
                    ...
                    for C in C_range:
                        for K in K_range:
                            ...
                            np.save('scores_rbf', scores)
                            np.save('cons_rbf', considered)
                            place += 1
                            ...
                            effective = 1/3
                            assigned = np.array([0 if scores[i]<0-np.log(0.5) else 1 for i in range(n_cons)], dtype= int)
                            ...
                            spec = str('(' + str('%.2f' % prior) + ', ' + str(prepre if prepre else 'raw  ') + ', ' + str(mPCA if mPCA==10 else (str(mPCA) + ' ' if mPCA else 'no')) + ', ' + str(str(mLDA) + ' ' if mLDA else 'no') + ', ' + str(lam) + '\t, ' + str(C) + '\t, ' + str(K) + '\t)')
